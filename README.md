...
2. Название файла должно совпадать с названием компонента.
3. Центр координат в центре первой ноги (при отсутсвии такой возможности центр координат в геометрическом центре футпринта).
...
# Решение проблемы с некорректным копирование свойств комнат
 ...
 Данный номер - это уникальный номер определяющий порядок расстановки компонентов из отдельной схемы на плате. Соответственно для мульти-канального проекта для различных каналов данные номера должны соответствовать для одинаковых компонентов. При модификации нередко происходит сбой данной нумерации для части каналов (особенно при изменении кол-ва каналов). Для решения проблемы необходимо привести данный номер в соответствие.
